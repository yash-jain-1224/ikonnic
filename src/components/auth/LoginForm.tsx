  ...
  return (
    <form onSubmit={submit} className="mx-auto max-w-md rounded-3xl border border-slate-200 bg-white p-7 shadow-card sm:p-9">
      ...
      <label className="mt-5 block text-xs font-black uppercase tracking-[0.13em] text-slate-500">
        Email or phone
        <input required name="identity" placeholder="you@example.com" className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3.5 text-sm normal-case tracking-normal outline-none focus:border-ikonnic-red" />
      </label>
      {mode !== "forgot" ? (
        <label className="mt-5 flex items-center gap-2 text-xs font-bold text-slate-600">
          <input type="checkbox" name="remember" className="size-4 accent-ikonnic-red" />
          Remember me on this device
        </label>
      ) : null}
      <button type="submit" className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-ikonnic-red px-6 py-3.5 text-sm font-black text-white hover:bg-red-700">
      ...
      <div className="mt-4 flex flex-wrap justify-center gap-3 text-xs font-bold">
        <button type="button" onClick={() => { setMode("login"); setSent(false); }} className="text-slate-600 hover:text-ikonnic-red">Login</button>
        <button type="button" onClick={() => { setMode("register"); setSent(false); }} className="text-slate-600 hover:text-ikonnic-red">Register</button>
        <button type="button" onClick={() => { setMode("forgot"); setSent(false); }} className="text-slate-600 hover:text-ikonnic-red">Forgot password</button>
      </div>
      ...
      <Link href="/account" className="mt-4 block text-center text-xs font-black text-ikonnic-red">Continue to account demo</Link>
    </form>
  );
}
