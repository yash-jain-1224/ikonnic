...
export function BlogCard({ post }: { post: BlogPost }) {
  return (
    <article className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-card">
      ...
      <div className="p-5">
        <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-giftora-red">{post.date}</p>
        <Link href={`/blog/${post.slug}`}><h2 className="mt-2 text-lg font-black leading-snug text-slate-900 group-hover:text-giftora-red">{post.title}</h2></Link>
        <p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-500">{post.excerpt}</p>
        <Link href={`/blog/${post.slug}`} className="mt-4 inline-flex items-center gap-1 text-xs font-black text-slate-800 hover:text-giftora-red">Read article <ArrowUpRight size={14} /></Link>
      </div>
    </article>
  );
}
