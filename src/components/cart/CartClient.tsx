        ...
        {items.map((item) => (
          <article key={item.lineId} className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row">
            ...
            <div className="min-w-0 flex-1">
              <Link href={`/product/${item.slug}`} className="font-black text-slate-950 hover:text-giftora-red">{item.title}</Link>
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                ...
                <button type="button" onClick={() => removeItem(item.lineId)} className="inline-flex items-center gap-1.5 rounded-full bg-red-50 px-3 py-2 text-xs font-bold text-giftora-red hover:bg-red-100">
      ...
      <div className="space-y-4">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
          ...
          <div className="mt-3 flex gap-2">
            <input defaultValue={coupon ?? ""} onBlur={(event) => applyCoupon(event.target.value)} placeholder="GIFTORA10" className="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-giftora-red" />
            <button type="button" onClick={() => applyCoupon("GIFTORA10")} className="rounded-xl bg-slate-950 px-4 py-2 text-xs font-black text-white">Apply</button>
          </div>
