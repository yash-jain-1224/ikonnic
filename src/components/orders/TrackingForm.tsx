    ...
    <div className="grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
      <form onSubmit={submit} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-card">
        ...
        <div className="mt-5 space-y-4">
          <label className="block text-xs font-bold text-slate-600">Order ID<input required name="orderId" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="GFT-2026-ABCDE" className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-ikonnic-red" /></label>
          <label className="block text-xs font-bold text-slate-600">Email or phone<input required name="identity" placeholder="Used during checkout" className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-ikonnic-red" /></label>
        </div>
        <button type="submit" className="mt-5 w-full rounded-full bg-ikonnic-red px-5 py-3.5 text-sm font-black text-white hover:bg-red-700">Track Order</button>
        ...
        {tracked ? (
          <>
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.15em] text-ikonnic-red">{matched ? matched.id : "Mock result"}</p>
          ...
          <div className="flex h-full min-h-72 flex-col items-center justify-center text-center">
            ...
            <p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">Submit the form to preview the full Ikonnic production and delivery journey.</p>
          </div>
        )}
      </div>
    </div>
