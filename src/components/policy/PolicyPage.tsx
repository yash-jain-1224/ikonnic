...
export function PolicyPage({ policyKey }: { policyKey: keyof typeof policies }) {
  ...
  return (
    <PageContainer className="py-10 sm:py-14">
      <article className="mx-auto max-w-3xl rounded-3xl border border-rosegold-200/60 bg-white p-7 shadow-card sm:p-10">
        ...
        <div className="mt-8 space-y-8">{policy.sections.map(([title, body]) => <section key={title}><h2 className="text-lg font-black text-slate-900">{title}</h2><p className="mt-2 whitespace-pre-line text-sm leading-7 text-slate-600">{body}</p></section>)}</div>
      </article>
    </PageContainer>
  );
}
