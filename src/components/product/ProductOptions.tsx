      ...
      {Object.entries(optionSets).map(([key, values]) => (
        <fieldset key={key}>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {values.map((value) => (
              <button key={value} type="button" onClick={() => update(key as keyof SelectedOptions, value)} className={`rounded-full border px-4 py-2 text-xs font-semibold transition ${options[key as keyof SelectedOptions] === value ? "border-ikonnic-red bg-red-50 text-ikonnic-red" : "border-slate-200 bg-white text-slate-700 hover:border-slate-300"}`}>
                {value}
              </button>
            ))}
          </div>
