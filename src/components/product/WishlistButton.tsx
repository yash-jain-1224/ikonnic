...
export function WishlistButton({ product, compact = false }: { product: Product; compact?: boolean }) {
  ...
  return (
    <button
      ...
      aria-pressed={liked}
      className={`inline-flex items-center justify-center gap-2 rounded-full border text-xs font-black transition ${compact ? "size-10 p-0" : "px-4 py-3"} ${liked ? "border-ikonnic-red bg-rosegold-50 text-ikonnic-red" : "border-rosegold-200/60 bg-white text-slate-600 hover:border-rosegold-300 hover:text-ikonnic-red"}`}
    >
